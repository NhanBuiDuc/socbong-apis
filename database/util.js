...
function readRowsAndConvertToJson(worksheet, headers, subheaders) {
  const data = [];
...
function getRowValues(worksheet, range) {
  ...
  for (let i = 0; i < header.length; i++) {
    ...
    if (cellValue !== sanitizeHeader(expectedMain) && expectedMain !== null) {
      if (acceptableHeaders[expectedMain]) {
        const acceptableValues =
          acceptableHeaders[expectedMain].map(sanitizeHeader);
        ...
        errors.push(
          `Expected main header "${expectedMain}" at index ${i}, found "${cellValue}"`
        );
      }
    }
    ...
    if (cellValue !== sanitizeHeader(expectedSub) && expectedSub !== null) {
      if (acceptableHeaders[expectedSub]) {
        const acceptableValues =
          acceptableHeaders[expectedSub].map(sanitizeHeader);
        if (!acceptableValues.includes(cellValue)) {
          errors.push(
        ...
        errors.push(
          `Expected sub header "${expectedSub}" at index ${i}, found "${cellValue}"`
        );
      }
    }
  }
...
function checkHeaderFile(
  ...
  const mainHeaderErrors = checkHeader(
    mainHeader,
    expectedMainHeader,
    null,
    acceptableHeaders
  ...
  const subHeaderErrors = checkHeader(
    subHeader,
    null,
    expectedSubHeader,
    acceptableHeaders
...
const transformDataKeys = (data, mapping) => {
...
function extractData(dictData, mapping) {
  const result = [];
    ...
    for (const entity in mapping) {
      ...
      for (const attribute in mapping[entity]) {
        ...
        if (
          ...
          const nonEmptyKeys = Object.keys(value).filter(
            (key) => value[key] !== ""
          );
